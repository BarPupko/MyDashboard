...
import { Download, Upload, Database, HardDrive, FileJson, AlertCircle, CheckCircle, Info } from 'lucide-react';
import { exportToJson, importFromJson, exportAllDataToJson, importAllDataFromJson } from '../utils/jsonExport';
import { useWorkHours } from '../hooks/useWorkHours';

export function BackupManager() {
  const { workDayEntries, summary, updateSummary, importWorkDayEntries } = useWorkHours();
  ...
  const handleExportFullBackup = () => {
    exportAllDataToJson();
    setImportStatus({ type: 'success', message: 'Full backup exported successfully!' });
    setTimeout(() => setImportStatus({ type: null, message: '' }), 3000);
  };

  const handleExportWorkHours = () => {
    exportToJson(workDayEntries, summary);
    setImportStatus({ type: 'success', message: 'Work hours exported successfully!' });
    setTimeout(() => setImportStatus({ type: null, message: '' }), 3000);
  };
  ...
  const handleImportFullBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.success && result.itemsImported) {
      const message = `Successfully restored backup!\n\n` +
        `- Todos: ${result.itemsImported.todos}\n` +
        `- Work Entries: ${result.itemsImported.workDayEntries}\n` +
        `- Settings: ${result.itemsImported.settings ? 'Yes' : 'No'}\n` +
        `- Theme: ${result.itemsImported.theme ? 'Yes' : 'No'}\n\n` +
  ...
  const handleImportWorkHours = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    if (result.success && result.data) {
      ...
      setImportStatus({ type: 'success', message: `Successfully imported ${result.data.workDayEntries.length} work entries!` });
    } else {
      setImportStatus({ type: 'error', message: `Import failed: ${result.error}` });
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">Export and import your data safely</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-8 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Current Data Overview</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <CheckCircle className="h-8 w-8 text-blue-600 dark:text-blue-400" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Total Todos</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{storageInfo.todos}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
              <Database className="h-8 w-8 text-purple-600 dark:text-purple-400" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Work Entries</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{storageInfo.workEntries}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">
                  {storageInfo.hasSettings ? 'Configured' : 'Default'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Full System Backup</h2>
          </div>
          
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mb-4 border border-blue-200 dark:border-blue-800">
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
              <strong>Recommended:</strong> Export everything in one file
            </p>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-500 mt-4">
            File format: <code className="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded">todo-dashboard-backup-YYYY-MM-DD.json</code>
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Work Hours Only</h2>
          </div>
          
          <div className="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
              Export or import only work hours data
            </p>
            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1 ml-4 list-disc">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-500 mt-4">
            File format: <code className="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded">work-hours-backup-YYYY-MM-DD.json</code>
          </p>
        </div>
        ...
        <div className="mt-8 p-6 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-amber-900 dark:text-amber-300 mb-2">Important Information</h3>
              <ul className="text-sm text-amber-800 dark:text-amber-400 space-y-1 list-disc ml-4">
                ...
                <li>When transferring to another computer, use the Full Backup option</li>
                <li>Importing will replace or merge existing data based on your choice</li>
